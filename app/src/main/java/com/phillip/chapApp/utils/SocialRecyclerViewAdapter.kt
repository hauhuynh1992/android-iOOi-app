...
class SocialRecyclerViewAdapter<T>(
    private val layout: Int,
    val onBind: (view: View, item: T, position: Int) -> Unit
) : RecyclerView.Adapter<SocialRecyclerViewAdapter<T>.ViewHolder<T>>() {
    var items = ArrayList<T>()
    var onItemClick: ((pos: Int, view: View, item: T) -> Unit)? = null
    var onItemLongClick: ((pos: Int, view: View, item: T) -> Unit)? = null
    ...
    inner class ViewHolder<T>(
        ...
        override fun onLongClick(v: View?): Boolean {
            onItemLongClick?.invoke(adapterPosition, v!!, items[adapterPosition])
            return true
        }
